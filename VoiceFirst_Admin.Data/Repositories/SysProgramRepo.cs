...
namespace VoiceFirst_Admin.Data.Repositories
{
    public class SysProgramRepo: ISysProgramRepo
    {
     ...
     CancellationToken cancellationToken)
        ...
        public async Task<(List<SysProgramLookUpDTO> Items, int TotalCount)>
    ...
    ");
            ...
            {
                sql.Append(" AND p.ApplicationId = @ApplicationId ");
                countSql.Append(" AND p.ApplicationId = @ApplicationId ");
                parameters.Add("ApplicationId", applicationId);
            }

            // 🔹 Search filter
            if (!string.IsNullOrWhiteSpace(basicFilterDto.SearchText))
            {
                sql.Append(" AND p.ProgramName LIKE @Search ");
                countSql.Append(" AND p.ProgramName LIKE @Search ");
                parameters.Add("Search", $"%{basicFilterDto.SearchText}%");
            }
    ...
    ");
            ...
            parameters.Add("Offset", (basicFilterDto.PageNumber - 1) * basicFilterDto.Limit);
            parameters.Add("Limit", basicFilterDto.Limit);

            using var connection = _context.CreateConnection();

            var items = (await connection.QueryAsync<SysProgramLookUpDTO>(
                new CommandDefinition(
                    sql.ToString(),
                    parameters,
                    cancellationToken: cancellationToken
