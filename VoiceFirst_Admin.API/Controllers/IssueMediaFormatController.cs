...
[ApiController]
[Route("api/issue-media-format")]
public class IssueMediaFormatController : ControllerBase
{
    private readonly ISysIssueMediaFormatService _svc;
    public IssueMediaFormatController(ISysIssueMediaFormatService svc) => _svc = svc;

    [HttpPost] public async Task<IActionResult> CreateAsync([FromBody] SysIssueMediaFormatCreateDTO m, CancellationToken ct) { var r = await _svc.CreateAsync(m, 1, ct); return StatusCode(r.StatusCode, r); }
    ...
    [HttpGet] public async Task<IActionResult> GetAllAsync([FromQuery] IssueMediaFormatFilterDTO f, CancellationToken ct) { var r = await _svc.GetAllAsync(f, ct); return StatusCode(r.StatusCode, r); }
    ...
    [HttpPatch("{id:int}")] public async Task<IActionResult> UpdateAsync(int id, [FromBody] SysIssueMediaFormatUpdateDTO m, CancellationToken ct) { if (id <= 0) return BadRequest(ApiResponse<object>.Fail(Messages.PayloadRequired, StatusCodes.Status400BadRequest, ErrorCodes.Payload)); var r = await _svc.UpdateAsync(m, id, 1, ct); return StatusCode(r.StatusCode, r); }
